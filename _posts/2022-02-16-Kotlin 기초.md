...
// 람다의 입력인자 타입을 지정해줄 땐 항상 ( ) 로 해줘야함
val sum: (Int) -> Int = {x:Int -> x}

// 반환값 없는 람다식 선언
val out: () -> Uint = {println("Hello world!")}
```
...
- with, apply, run, also, let 함수
    ...
    - apply
      ...
      ```kotlin
      fun main(){
          val f: Fruit = Fruit("APPLE", 100)
          var test = f?.apply{
              f.name = "apple"
              f.price = 1500
              }
              println("f = ${f.name}, ${f.price}")      // 얘도 apple, 1500으로 나옴
              println("test = ${test.name}, ${test.price}")
        }
