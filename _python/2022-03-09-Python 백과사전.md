...
모듈: .py 파일  패키지: 디렉토리. 디렉토리 안에 여러개의 모듈을 넣어둘 수 있다.

- `import`를 사용할 때 기억해야 할 점
  - `import` 뒤에 오는 `name`{:.success}은 사용할 때 항상 그 `name`{:.success}을 **그대로** 사용해 줘야 한다.
    ```python
    import numpy

    numpy.exp([1, 3, 5])    # numpy라는 name을 그대로 사용함
    ```
  - `import`만 사용될 때는 `디렉토리, 모듈`{:.warning} 만 올 수 있지만 `from`과 같이 연계하여 사용할 경우 `함수, 변수, 클래스`{:.warning}도 올 수 있다.  
  ...
  - `from [디렉토리] import *`: from name의 마지막이 **디렉토리**이며 뒤 따라오는 import가 * 일 경우
  **\_\_init\_\_.py** 스크립트를 활용해야 한다.  `from [디렉토리] import *`를 만나면 아래 문구만 기억하면 편하다.
